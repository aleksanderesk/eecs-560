...
ClosedMap<ItemType>::ClosedMap(const int size) : tableSize(size) {
    table = new Node<ItemType>[size];
    for (int i = 0; i < size; i++) {
        table[i] = Node<ItemType>();
    }
}
 ...
 */
template<class ItemType>
int ClosedMap<ItemType>::hash(const ItemType& value, const int iter) const {
...
bool ClosedMap<ItemType>::insert(const ItemType& newValue) const {
    if (contains(newValue) != -1) {
        std::cout << "Item already exists in map" << std::endl;
        return false;
    }
    else {
        int iter = 0;
